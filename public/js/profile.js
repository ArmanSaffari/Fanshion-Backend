window.addEventListener("DOMContentLoaded", async function () {
  ...
  async function displayUserDetails(userDetails) {
    ...
    $('#ageInput').val(userDetails.age);
    $('#ageDisplay').text(userDetails.age);
  ...
  function addSavedItemCard(product) {
    ...
    cardItem.html(`
      <div class="card border border-dark col-12" style="height: 350px">
        <a href="/itemDetails/${product.id}" class="h-100 text-decoration-none text-dark ">
  ...
  $('#ageInput').on('change', function() {
    $('#ageDisplay').text($('#ageInput').val());
  });

  
  $('#DOBInput').on('change', function() {
    let today = new Date();
    let birthDate = new Date($('#DOBInput').val());
